...
{
    private const EXIF_TEST_DATA = <<<BYTES
    ...
    {
        $chunk = Exif::forBytes(0, self::EXIF_TEST_DATA);
        $parsedExif = $chunk->getParsedExif();

        self::assertNotNull($parsedExif);

        self::assertArrayHasKey('IFD0', $parsedExif);
        self::assertArrayHasKey('Model', $parsedExif['IFD0']);
        self::assertEquals($parsedExif['IFD0']['Model'], 'Canon EOS 40D');
    }

    public function testDecodesExifDataAsOneArray(): void
    {
        $chunk = Exif::forBytes(0, self::EXIF_TEST_DATA);
        $parsedExif = $chunk->getParsedExif(false);
